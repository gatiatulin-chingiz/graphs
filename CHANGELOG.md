...
- Отчёт `show_degree_report`: колонка «компонент после», сценарии с/без юрлиц, краткий итог по текущему `config`.
- Config: `hub_degree_n`, `remove_legal_entities` (с развёрнутыми комментариями); коннекторы БД → `src/connections.py`.
